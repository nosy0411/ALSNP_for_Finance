...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3) {
        printf("%s <mmap filename> <# MiB>\n", argv[0]);
        exit(1);
    }

    int flags = MAP_SHARED;
    /* HUGETLBMOUNT_PATH : diretory on hugetlbfs. 
     * Some linux distributions provide a hugetlbfs mount point on /dev/hugepages. */
#define HUGETLBMOUNT_PATH "/var/lib/hugetlbfs/global/pagesize-2MB"
    char path[0xff];
    snprintf(path, sizeof(path), "%s/%s", HUGETLBMOUNT_PATH, argv[1]);
    ...
    p_array = mmap(NULL, sz_map, PROT_READ|PROT_WRITE, flags, fd, 0);
