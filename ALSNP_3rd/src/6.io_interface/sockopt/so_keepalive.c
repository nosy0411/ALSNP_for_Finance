/* vim: set ts=4 sw=4: */
/* Filename    : so_keepalive.c
 * Description : SO_KEEPALIVE socket option for tcp
...
void start_child(int sfd, int idx)
{
	...
	for(;;) {
		...
		sockopt = 1;
		if (setsockopt(cfd, SOL_SOCKET, SO_KEEPALIVE, &sockopt, sizeof(sockopt))) {
			pr_err("[TCP server] : Fail: setsockopt()"); 
			exit(EXIT_FAILURE);
		}
		/* first Idle time */
		sockopt = 20;
		if (setsockopt(cfd, IPPROTO_TCP, TCP_KEEPIDLE, &sockopt, sizeof(sockopt))) {
			pr_err("[TCP server] : Fail: setsockopt()"); 
			exit(EXIT_FAILURE);
		}
		/* interval time to send probe packet after idle time */
		sockopt = 10;
		if (setsockopt(cfd, IPPROTO_TCP, TCP_KEEPINTVL, &sockopt, sizeof(sockopt))) {
			pr_err("[TCP server] : Fail: setsockopt()"); 
			exit(EXIT_FAILURE);
		}
