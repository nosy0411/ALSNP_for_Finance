    ...
    while (1) {
        ...
        for (i=1; i<cnt_fd_socket && ret_poll > 0; i++) {
            if (pollfds[i].revents & POLLIN) {
                ...
                if (ioctl(pollfds[i].fd, FIONREAD, &ret_ionread) == -1) {
                    pr_err("ioctl(%d, FIONREAD, ..) (%s)", pollfds[i].fd, strerror(errno));
                } else {
                    pr_out("fd(%d) : socket buffer(%d byte)", pollfds[i].fd, ret_ionread);
                }
#endif
                if ((ret_recv = recv(pollfds[i].fd, buf, 2, 0)) == -1) { /* test Level Trigger */
                    ...
                    if (ret_recv == 0) {
                        ...
                        i--;	/* del_socket에 의해 전체 개수가 감수했으므로. */
                    } else {
                        /* normal */
    ...
    for (i=0; i<cnt_fd_socket; i++) {
        if (pollfds[i].fd == fd) {
            if (i != (cnt_fd_socket-1)) { 
                pollfds[i] = pollfds[cnt_fd_socket-1]; /* replace fd to last one */
            }
