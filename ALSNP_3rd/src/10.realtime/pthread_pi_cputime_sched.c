/* vim: set ts=4 sw=4: */
/* Filename    : pthread_pi_cputime_sched.c
 * Description : calculate pi with specific scheduler
...
int main(int argc, char *argv[])
{
    ...
    switch((char)atoi(argv[1])) {
        case 1:
            sched_set_options(SCHED_FIFO, 1);
            break;
        case 2:
            sched_set_options(SCHED_RR, 2);
            break;
        case 3:
            sched_set_options(SCHED_BATCH, 11);
            break;
        case 5:
            sched_set_options(SCHED_IDLE, 10);
            break;
       default:
            sched_set_options(SCHED_OTHER, 10);
            break;
    }
